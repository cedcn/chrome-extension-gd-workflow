...
const allLabelKeys = ['not[label_name][]', 'label_name[]']
...
const milestoneDropdownItems = milestoneTitles.map(function(milestoneTitle) {
  const current = getCurrentMileStoneStr()

  return {
    itemText: current === milestoneTitle ? `^${milestoneTitle}` : milestoneTitle,
...
const allProjectValues = ['主线改版', '移动端增长', '质量部']
...
const allTypeValues = ['bug', 'feature', 'defect']
